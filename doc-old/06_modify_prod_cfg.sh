...
echo "开始修改dubbo的版本号"
sed -i 's/param_dubboxVersion/'$param_dubboxVersion'/' `grep "param_dubboxVersion" -rl --include="application-prod.yaml" */`

echo "开始修改dubbo服务提供者host和xxl-job客户端ip"
sed -i 's/param_product_center_ip/'$param_product_center_ip'/' `grep "param_product_center_ip" -rl --include="application-prod.yaml" */`
sed -i 's/param_sales_center_ip/'$param_sales_center_ip'/' `grep "param_sales_center_ip" -rl --include="application-prod.yaml" */`
sed -i 's/param_author_center_ip/'$param_author_center_ip'/' `grep "param_author_center_ip" -rl --include="application-prod.yaml" */`
sed -i 's/param_finance_center_ip/'$param_finance_center_ip'/' `grep "param_finance_center_ip" -rl --include="application-prod.yaml" */`
sed -i 's/param_operator_center_ip/'$param_operator_center_ip'/' `grep "param_operator_center_ip" -rl --include="application-prod.yaml" */`
...
sed -i 's/\/v1\//\/'$rest_version'\//g' `grep "/v1/" -rl --include="application-prod.yaml" */`
